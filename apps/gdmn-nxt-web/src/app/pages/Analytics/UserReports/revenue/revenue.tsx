...
import styles from './revenue.module.less';
...
export function Revenue(props: RevenueProps) {
  ...
  const filterEntityName = 'revenue';
  ...
  const { data, isFetching, refetch } = useGetRevenueQuery({ onDate: onDate || [new Date(), new Date()], options }, { skip: !show });
  ...
  return (
    ...
    >
      ...
      {show ? <RevenueReport data={data} isFetching={isFetching} /> : null}
    </Stack>
  );
}
