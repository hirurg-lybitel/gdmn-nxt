  ...
  endpoints: (builder) => ({
    getCustomerContracts: builder.query<ICustomerContractWithID[], number | void>({
      ...
      async onQueryStarted() {
        console.log('⏩ request', 'GET', `${baseUrlApi}customercontracts`);
      },
    ...
    updateCustomerContract: builder.mutation<ICustomerContractsRequestResult, Partial<ICustomerContractWithID>>({
      async onQueryStarted({ ID: id }) {
        console.log('⏩ request', 'PUT', `${baseUrlApi}customerContracts/${id}`);
      },
      query(body) {
        const { ID: id } = body;
        return {
    ...
    addCustomerContract: builder.mutation<ICustomerContractWithID[], Partial<ICustomerContractWithID>>({
      async onQueryStarted() {
        console.log('⏩ request', 'POST', `${baseUrlApi}customerContracts`);
      },
      query(body) {
        return {
    ...
    deleteCustomerContract: builder.mutation<{id: number}, number>({
      async onQueryStarted(id) {
        console.log('⏩ request', 'DELETE', `${baseUrlApi}customerContracts/${id}`);
      },
      query(id) {
        return {
          url: `customerContracts/${id}`,
