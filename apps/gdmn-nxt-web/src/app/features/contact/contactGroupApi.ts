  ...
  endpoints: (builder) => ({
    getGroups: builder.query<IContactHierarchy[], void>({
      ...
      async onQueryStarted() {
        console.log('⏩ request', 'GET', `${baseUrlApi}contactgroups`);
      },
    ...
    updateGroup: builder.mutation<IContactGroupsRequestResult, Partial<IContactHierarchy>>({
      async onQueryStarted({ ID: id }) {
        console.log('⏩ request', 'PUT', `${baseUrlApi}contactgroups/${id}`);
      },
      query(body) {
        const { ID: id } = body;
        return {
    ...
    addGroup: builder.mutation<IContactHierarchy[], Partial<IContactHierarchy>>({
      async onQueryStarted() {
        console.log('⏩ request', 'POST', `${baseUrlApi}contactgroups`);
      },
      query(body) {
        return {
          url: 'contactgroups',
    ...
    deleteGroup: builder.mutation<{id: number}, number>({
      async onQueryStarted(id) {
        console.log('⏩ request', 'DELETE', `${baseUrlApi}contactgroups/${id}`);
      },
      query(id) {
        return {
          url: `contactgroups/${id}`,
