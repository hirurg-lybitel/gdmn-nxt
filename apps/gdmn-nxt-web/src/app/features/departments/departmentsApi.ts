  ...
  endpoints: (builder) => ({
    getDepartments: builder.query<IContactWithID[], number | void>({
      ...
      async onQueryStarted() {
        console.log('⏩ request', 'GET', `${baseUrlApi}departments`);
      },
    ...
    updateDepartment: builder.mutation<IDepartmentsRequestResult, Partial<IContactWithID>>({
      async onQueryStarted({ ID: id }) {
        console.log('⏩ request', 'PUT', `${baseUrlApi}departments/${id}`);
      },
      query(body) {
        const { ID: id } = body;
        return {
    ...
    addDepartment: builder.mutation<IContactWithID[], Partial<IContactWithID>>({
      async onQueryStarted() {
        console.log('⏩ request', 'POST', `${baseUrlApi}departments`);
      },
      query(body) {
        return {
          url: 'departments',
    ...
    deleteDepartment: builder.mutation<{id: number}, number>({
      async onQueryStarted(id) {
        console.log('⏩ request', 'DELETE', `${baseUrlApi}departments/${id}`);
      },
      query(id) {
        return {
          url: `departments/${id}`,
