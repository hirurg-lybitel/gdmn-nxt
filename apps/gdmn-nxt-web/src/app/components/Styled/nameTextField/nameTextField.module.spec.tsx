...
import { NameTextField, NameTextFieldTest } from './nameTextField';
import userEvent from '@testing-library/user-event';
describe('NameTextField', () => {
  it('should render successfully', () => {
    const { baseElement } = render(<NameTextField />);
    expect(baseElement).toBeTruthy();
  });
  const cummonTests = (type: 'string'|'object') => {
    const user = userEvent.setup();
    it(type + 'Value: set main value and clear', async () => {
      const { baseElement } = render(<NameTextFieldTest type={type} />);
    ...
    it(type + 'Value: open/close popup', async () => {
      const { baseElement, getByTestId, queryByTestId } = render(<NameTextFieldTest type={type} />);
      // input -> away
      await user.click(baseElement.getElementsByTagName('input')[0]);
