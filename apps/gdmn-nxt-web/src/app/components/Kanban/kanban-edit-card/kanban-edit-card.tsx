...
export function KanbanEditCard(props: Readonly<KanbanEditCardProps>) {
  ...
  return (
    <EditDialog
      ...
      fullwidth
      title={formik.values.ID > 0 ? `Редактирование сделки №${card?.DEAL?.USR$NUMBER ?? 'Н/Д'}: ${card?.DEAL?.USR$NAME ?? 'Без имени'}` : 'Создание сделки'}
      form="mainForm"

      deleteButtoHint={`${tasksExist ? 'Нельзя удалить сделку с задачами' : ''}`}
      showDeleteButtonHintAnyway
      deleteConfirmTitle="Удаление сделки"
      deleteButtonDisabled={tasksExist}
      onDeleteClick={handleDeleteClick}
      deleteButton={(!!card?.DEAL?.ID && (card?.DEAL?.ID > 0)) && deleteable && checkDeletePermissions}

      submitButtonDisabled={customerFetching || denyReasonsIsFetching || isFetchingCard}
      submitButton={formik.values.ID > 0 ? userPermissions?.deals.PUT : userPermissions?.deals.POST}
    >
