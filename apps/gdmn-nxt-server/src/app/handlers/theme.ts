...
const eintityName = 'TgdcAttrUserDefinedUSR_CRM_MODE';
...
const upsert: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    const query = {
      name: 'mode',
      query: `
        UPDATE OR INSERT INTO USR$CRM_MODE(${actualFieldsNames})
        VALUES(${paramsString})
        MATCHING(ID)
...
const remove: RequestHandler = async(req, res) => {
  ...
  try {
    result = await attachment.executeQuery(
      ...
      BEGIN
        SUCCESS = 0;
        FOR SELECT ID FROM USR$CRM_MODE WHERE ID = :ID INTO :LAB_ID AS CURSOR curMODE
        DO
        BEGIN
          DELETE FROM USR$CRM_MODE WHERE CURRENT OF curMODE;
