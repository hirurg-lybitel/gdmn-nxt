...
import { FindHandler, FindOperator, IDealFeedbackSatisfactionRate } from '@gsbelarus/util-api-types';

const find: FindHandler<IDealFeedbackSatisfactionRate> = async (
  sessionID,
  clause
) => {
  ...
  try {
    ...
    const rows = await fetchAsObject(
      `SELECT
        z.ID,
        z.USR$NAME
      FROM USR$CRM_FEEDBACK_RATING z
      ${clauseString.length > 0 ? ` WHERE ${clauseString}` : ''}
      ORDER BY z.USR$NUMBER`,
      { ...whereClause });

    const entities = rows.map<IDealFeedbackSatisfactionRate>(r => ({
      id: r['ID'],
      name: r['USR$NAME'],
    }));
...
export const satisfactionRateFeedbackRepository = {
  find,
};
