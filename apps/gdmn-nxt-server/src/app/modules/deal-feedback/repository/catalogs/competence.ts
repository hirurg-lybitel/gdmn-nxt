...
import { FindHandler, FindOperator, IDealFeedbackCompetence } from '@gsbelarus/util-api-types';

const find: FindHandler<IDealFeedbackCompetence> = async (
  sessionID,
  clause
) => {
  ...
  try {
    ...
    const rows = await fetchAsObject(
      `SELECT
        z.ID,
        z.USR$NAME
      FROM USR$CRM_FEEDBACK_COMPETENCE z
      ${clauseString.length > 0 ? ` WHERE ${clauseString}` : ''}
      ORDER BY z.USR$NUMBER`,
      { ...whereClause });

    const entities = rows.map<IDealFeedbackCompetence>(r => ({
      id: r['ID'],
      name: r['USR$NAME'],
    }));
...
export const competenceFeedbackRepository = {
  find,
};
